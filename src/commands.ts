...
system.beforeEvents.startup.subscribe((ev) => {
    ...
    customCommand.registerCommand({
        ...
        if (arg === "info") BedrockVote.sendInfoMessage(player);
        else if (arg === "version")  BedrockVote.sendVersionMessage(player);
        else if (arg === "claim") BedrockVote.claimVote(player); 
        else (async () => {
            const status = await VoteRequest.checkVoteStatus(player.name);

            if (status === VoteStatus.NotFound) return BedrockVote.sendInfoMessage(player);
            if (status === VoteStatus.Claimed) return BedrockVote.sendClaimedMessage(player);
